...
public class SalesOrderDetailsServiceImpl implements SalesOrderDetailsService {
    ...
    @Override
    public List<SalesOrderDetails> selectSalesReturnGoodsList(int salesOrderId,String partsCd,String goodsName) {
        try{
            List<SalesOrderDetails>  salesOrderDetailsList = salesOrderDetailsMapper.selectSalesReturnGoodsList(salesOrderId,partsCd,goodsName);
    ...
    @Override
    public int selectSalesReturnGoodsCount(int salesOrderId,String partsCd,String goodsName) {
        try{
            return salesOrderDetailsMapper.selectSalesReturnGoodsCount(salesOrderId,partsCd,goodsName);
        }catch (Exception e){
            e.printStackTrace();
        }
        return 0;
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public   List<ShipmentDetails>  updateSaleOrderDetailsQuantity(ShipmentListDto shipmentListDto) {
        //总箱数
        ...
        for (SalesOrderDetailsListDto salesOrderDetailsListDto:shipmentListDto.getSalesOrderDetailsListDtos()
             ) {
            ...
            if(salesOrderDetailsListDto.getOutQuantity().compareTo(salesOrderDetails.getNotOutQuantity())>0){
                throw new MyException("导出失败,出货数量应小于或等于需求数量");
            }
            BigDecimal outQuantity  =salesOrderDetailsListDto.getOutQuantity().add(salesOrderDetails.getOutQuantity());
            BigDecimal notOutQuantity = salesOrderDetails.getNotOutQuantity().subtract(salesOrderDetailsListDto.getOutQuantity());
      ...
      /*  if(salesOrderId>0){
        ...
        if(shipmentListDto.getShipmentNumber()!=null){
            shipment.setShipmentNumber(shipmentListDto.getShipmentNumber());
        }else{
            //发货单编号
