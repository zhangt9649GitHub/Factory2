...
layui.use(['form', 'table', 'layer', 'laydate'], function() {
	...
	function getPurchaseTypeList(){
		return axios.post('/purchaseOrder/getPurchaseTypeList')
	}

	// 获取销售订单
	function getSalesOrder() {
		return axios.post('/purchaseOrder/getSalesOrder')
	}

	// 获取编辑信息
	let oldSupplierId; // 供应商选中的值
	axios.all([getSupplierNameList(),getPurchaseTypeList(),getSalesOrder()])
		.then(axios.spread(function(supplierNameList,purchaseTypeList,salesOrder){
			if(supplierNameList.data.code == 0){
				$.each(supplierNameList.data.data, function(i,item){
					$('select[name="supplierId"]').append(`<option value="${item.supplierId}">${item.supplierName}</option>`)
				});

			}
			if(purchaseTypeList.data.code == 0){
				$.each(purchaseTypeList.data.data, function(i,item){
					$('select[name="purchaseType"]').append(`<option value="${item.value}">${item.bizName}</option>`)
				});
			}
			if(salesOrder.data.code == 0){
				$.each(salesOrder.data.data, function(i,item){
	...
	form.on('submit(*)', function(data) {
		...
		axios.post('/purchaseOrder/updatePurchaseOrderInfo', data.field)
			.then(function (response) {
...
var active = {
	...
	// 确认合同
	confirmContract: function (that) {
		// 禁止点击
		$(that).addClass('zq-submit-disabled');
		let formdata = new FormData();
		formdata.append('purchaseOrderId',$('input[name="purchaseOrderId"]').val());

		axios.post('/purchaseOrder/affirmationContract',formdata)
			.then(function(response){
				layer.msg(response.data.msg);
				if (response.data.code == 20005) {
					setTimeout(function () {
