...
layui.use(['form', 'table', 'layer', 'laydate'], function() {
	...
	//获取编号
	axios.get('/general/getFormNumber?type=18'
	).then(function (response) {
		let data = response.data;
		if(data.code == 0) {
			$('input[name="purchaseReturnOrderNumber"]').val(data.data.number);
		}
	});
	...
	form.on('submit(*)', function(data) {
		...
		axios.post('/purchaseReturnOrder/insertPurchaseReturnOrder', data.field)
			.then(function (response) {
