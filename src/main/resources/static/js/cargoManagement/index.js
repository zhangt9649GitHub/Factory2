	...
	layui.use(['form', 'table', 'layer','upload'], function() {
		...
		form.on('submit(*)', function(data){
			...
			table.reload('idTest', {
				...
				,where: {
                    goodsNumber: $.trim(data.field.goodsNumber),
		...
		var active = {
			...
			// 导出
			export: function(){
				location.href= '/goods/exportGoodsList?partsCd='+$.trim(exportField.partsCd)+'&goodsName='+ $.trim(exportField.goodsName)
					+'&variety='+ $.trim(exportField.variety) +'&unit='+ $.trim(exportField.unit) +'&specification='+ $.trim(exportField.specification)
					+'&weight='+ $.trim(exportField.weight) +'&unitPrice='+ $.trim(exportField.unitPrice)+'&aPrice='+ $.trim(exportField.aPrice)
					+'&bPrice='+ $.trim(exportField.bPrice)+'&sellingPrice='+ $.trim(exportField.sellingPrice)
					+'&wholesalePrices='+ $.trim(exportField.wholesalePrices)+'&minimumStock='+ $.trim(exportField.minimumStock)
					+'&japanStock='+ $.trim(exportField.japanStock)+'&tjStock='+ $.trim(exportField.tjStock)+'&goodsNumber='+ $.trim(exportField.goodsNumber);
			},
			// 重置
			reset: function () {
            ...
            printAll(){
                layer.confirm('是否打印？', {
                    ...
                    axios.post('/goods/updateAllGoodsPrint')
                        .then(function (response) {
                            layer.msg(response.data.msg)
                        })
