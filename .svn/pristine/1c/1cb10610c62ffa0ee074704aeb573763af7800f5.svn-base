...
public interface ProductionInstructionGoodsMapper {
     ...
     * @return
     */
    List<KucunGoods> selectJPExportsGoodsList(@Param("goodsId") Integer goodsId,@Param("partsCd") String partsCd,@Param("goodsName") String goodsName);
     ...
     * @return
     */
    int  selectJPExportsGoodsCount(@Param("goodsId") Integer goodsId,@Param("partsCd") String partsCd,@Param("goodsName") String goodsName);

    /**
     * 根据注文书中的回次查询生产知识书中的Id,然后查询生产指示书货物表中的Id下的内容
     * @param productionInstructionId
     * @return
     */
    List<ProductionInstructionGoods> selectProInsGoodsInfoByNDProid(@Param("productionInstructionId") Integer productionInstructionId);
}