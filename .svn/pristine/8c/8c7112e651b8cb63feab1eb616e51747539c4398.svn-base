...
layui.use(['form', 'table', 'layer', 'laydate'], function() {
	...
	//获取编号
	axios.get('/general/getFormNumber?type=11'
	).then(function (response) {
		let data = response.data;
		if(data.code == 0) {
			$('input[name="refundPickingNumber"]').val(data.data.number);
		}
	});

	// 获取批号
	axios.get('/general/getSerialNumberList'
	).then(function (response) {
		let data = response.data;
		if(data.code == 0) {
	...
	form.on('submit(*)', function(data) {
		...
		for(let i in tableBak){
			...
			if(tableBak[i].goodsId){
				...
				if(isNaN(tableBak[i].quantity)){
					layer.msg(`第${index}行的数量只能是数字`, {icon: 5});
					return false;
				}else if( Number(tableBak[i].quantity) <= 0 ){
					layer.msg(`第${index}行数量不能小于等于0`, {icon: 5});
					return false;
				}else if(Number(tableBak[i].quantity) > Number(tableBak[i].skgCount)){
					layer.msg(`第${index}行数量不能大于库存数`, {icon: 5});
					return false;
				}else{
					let arr ={
						...
						reason: (tableBak[i].comment)?tableBak[i].comment:'',
					};
		...
		if(refundPickingDetailsList.length == 0){
			layer.msg('请选择货物', {icon: 5});
			return false;
		}
		data.field.refundPickingDetailsList = refundPickingDetailsList;
		// 禁止点击
		$(that).addClass('zq-submit-disabled');

		axios.post('/refundPicking/insertPicking', data.field)
			.then(function (response) {
				layer.msg(response.data.msg);
				if (response.data.code == 10002) {
					setTimeout(function () {
						// active.zq_return();
						location.href="/admin/returnOrder/index";  //刷新父页面
					}, 1000);
				}else {
					// 取消禁止点击
...
var active = {
	zqIframeShow: function (dataIndex) {
		...
		let url = `/admin/returnOrder/showTable?dataIndex=${dataIndex}&warehouseId=${val}`;
		if(val.length != 0){
			zqIframeShow('查看',url,700,520)
		}else{
			layer.msg('请选择退出仓库', {icon: 5});
		}
