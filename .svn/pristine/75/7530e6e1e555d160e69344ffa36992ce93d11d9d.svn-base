...
layui.use(['form', 'table', 'layer', 'laydate'], function() {
	...
	//获取编号
	axios.get('/general/getFormNumber?type=1'
	).then(function (response) {
		let data = response.data;
		if(data.code == 0) {
			$('input[name="requisitionNumber"]').val(data.data.number);
		}
	...
	//获取调拨类型
	axios.post('/requisition/getRequisitionTypeList'
	).then(function (response) {
		let data = response.data;
		if(data.code == 0) {
	...
	// 获取动态表格的数据
		...
		for(let i in tableBak){
			if(tableBak[i].goodsId){
				if(isNaN(tableBak[i].quantity)){
					...
					requisitionDetailsList.push(arr);
				}
			}
		...
		if(requisitionDetailsList.length == 0){
			layer.msg('请选择调拨货物', {icon: 5});
			return false;
		}
		data.field.requisitionDetailsList = requisitionDetailsList;
		// 禁止点击
		$(that).addClass('zq-submit-disabled');

		axios.post('/requisition/insertRequisition', data.field)
			.then(function (response) {
				layer.msg(response.data.msg);
				if (response.data.code == 10002) {
					setTimeout(function () {
						// active.zq_return();
						location.href="/admin/requisition/index";  //刷新父页面
					}, 1000);
				}else {
					// 取消禁止点击
...
var active = {
	zqIframeShow: function (dataIndex) {
		...
		let url = `/admin/requisition/showTable?dataIndex=${dataIndex}&outWarehouseId=${val}`;
		if(val.length != 0){
