		...
		var active = {
			add: function(that, href) {
				...
				if (href) {
					let url;
					if(row){
						url= href + '?parentId=' + row.parentId;
						console.log(url);
						zqIframeView('新增',url,400,550)
					}else {
						url = href + '?parentId=0';
						console.log(url);
						zqIframeView('新增',url,400,550)
			...
			delete: function() {
				if (row) {
					layer.confirm('你确定删除数据吗？如果存在下级节点则一并删除，此操作不能撤销！', {
						...
						formdata.append('id', parseInt(row.id));
						axios.post('/bizd/delete',formdata
						).then(function (response) {
							layer.close(index);
