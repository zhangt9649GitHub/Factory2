...
public class GoodsManagementController {
    ...
    @ApiOperation(value = "货物一键打印", notes = "货物一键打印", tags = {"@郝腾"})
    @RequestMapping(value = "/updateAllGoodsPrint", method = RequestMethod.POST)
    @ResponseBody
    public LayuiCommonResponse updateAllGoodsPrint() {
        LayuiCommonResponse response = new LayuiCommonResponse();
        try {
            goodsService.updateGoodsPrint();
            response.setMsg(HtCode.SUCCESS_PRINTING.getInfo());
            response.setCode(HtCode.SUCCESS_PRINTING.getCode());
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @ApiOperation(value = "导入货物日本数据", notes = "导入货物日本数据", tags = {"@郝腾"})
    @RequestMapping(value = "/importGoodsInfo", method = RequestMethod.POST)
    @ResponseBody
    public LayuiCommonResponse importGoodsInfo(@ApiParam(value = "MultipartFile", required = true) MultipartFile file) {
        LayuiCommonResponse response = new LayuiCommonResponse();
        try {
            goodsService.importGoodsInfo(file);
            response.setCode(HtCode.SUCCESS_IMPORT.getCode());
            response.setMsg(HtCode.SUCCESS_IMPORT.getInfo());
        } catch (Exception e) {
            e.printStackTrace();
