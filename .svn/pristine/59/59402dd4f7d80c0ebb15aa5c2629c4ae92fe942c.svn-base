$(document).ready(function(){
	layui.use(['form', 'table', 'layer', 'laydate',"table"], function() {
		...
		function getData(item){
			...
			formdata.append('purchaseApplyDetailsId',item);
			return axios.post('/purchaseApply/getPurchaseApplyDetailsById',formdata)
		...
		(function(){
			...
			let detailsId = $('input[name="detailsId"]').val();
			let detailsIdList = detailsId.split(',');

			// 接口循环
			let getDataList = [];
			$.each(detailsIdList,function(i,item){
				getDataList.push(getData(item));

			});
			// 多个接口的返回信息集合到一起
			axios.all(getDataList).then(function(response){
				let goodsList = [];
				$.each(response,function (i,item) {
					let data = item.data.data;
					let arr = {
						'purchaseApplyDetailsId': data.purchaseApplyDetails.purchaseApplyDetailsId,
						...
						'purchasePrice': data.purchaseApplyDetails.goods.aPrice,
						'taxIncludedAmount': (Number(data.purchaseApplyDetails.quantity)* Number(data.purchaseApplyDetails.goods.aPrice)).toFixed(2)
					};
					goodsList.push(arr);
					form.val('formTest',{
						'supplierId': data.purchaseApplyDetails.goods.supplier.supplierId,
						'supplierName': data.purchaseApplyDetails.goods.supplier.supplierName
					})
				});
				table.reload('idTest', {
					data: goodsList
		...
		form.on('submit(*)', function(data) {
			...
			for(let i in tableBak){
				if(tableBak[i].goodsId){
					let arr ={
						purchaseApplyDetailsId: tableBak[i].purchaseApplyDetailsId,
			...
			axios.post('/purchaseApply/createPurchaseOrder', data.field)
				.then(function (response) {
