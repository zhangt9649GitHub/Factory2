...
//    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
    public void updateNoteDocumentData() {
        ...
        // 2. 修改发注残
        // 查询注文书内容
            /*NoteDocumentDto noteDocumentDtoBySelect = new NoteDocumentDto();
            List<NoteDocumentDto> noteDocumentDtoList2 = noteDocumentMapper.selectNoteDocumentList(noteDocumentDtoBySelect);
        ...
        //3.修改引当残
        NoteDocumentDto noteDocumentDtoBySelect = new NoteDocumentDto();
        List<NoteDocumentDto> noteDocumentDtoList3 = noteDocumentMapper.selectNoteDocumentList(noteDocumentDtoBySelect);
        for (NoteDocumentDto noteDocumentDto4 : noteDocumentDtoList3) {
            // 2. 修改发注残
            //将注文书中重复的货物的数量和合计金额进行叠加
            NoteDocument noteDocument2 = noteDocumentMapper.selectAllCountByGoodsId(noteDocumentDto4.getGoodsId(), state);
            //修改注文书发注残字段
            noteDocumentMapper.updateCountByGoodsId(noteDocument2.getCount(), noteDocumentDto4.getGoodsId(), state);   //根据货物id和注文书状态1.无采购修改注文书中货物发注残
                ...
                if (!(productionInstructionDetailList.isEmpty())) { //如果在生产指示书详情中根据生产指示书货物Id和货物Id查询到数据的话
                    for (ProductionInstructionDetail productionInstructionDetail : productionInstructionDetailList) {
                        ...
                        if (productionInstructionGoods.getProductionSchedule().compareTo(BigDecimal.ZERO) != 0) {
                            //引当残=生产必要数(生产指示书详情)/生产预定数(生产指示书货物)*未完成数量(生产指示书货物)
                            ...
                            System.out.println("所有的值是:" + productionInstructionDetail.getProductionSchedule() + "--" + productionInstructionGoods.getProductionSchedule() + "--" + productionInstructionGoods.getUnfinishedQuantity() + "--" + yinDangCan);
                            //根据注文书Id查询引当残,如果不为空,进行叠加,否则直接修改引当残的值
...
//                                System.out.println("输出结果是--ndid:" + noteDocumentDto4.getNdid() + ",goodsId:" + productionInstructionDetail.getGoodsId() + ",yinDangCan;" + yinDangCan);
                            ...
                            KucunGoods goodsKuCun = stockKucunGoodsMapper.getStockKucunGoodsCountByGoodsId(productionInstructionDetail.getGoodsId());
                            if (goodsKuCun != null) {
                                BigDecimal annotation = yinDangCan.subtract(goodsKuCun.getSkgCount().add(noteDocumentDto4.getCount()));
                                noteDocumentMapper.updateAnnoByNdId(noteDocumentDto4.getNdid(), productionInstructionDetail.getGoodsId(), annotation);
                            }
                        }
                    }
                }
